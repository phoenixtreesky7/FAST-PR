...
def show_cam_on_image(img, mask,name):
    ...
    cam = cam / np.max(cam)
    cv2.imwrite(name+"cam.jpg", np.uint8(255 * cam))
...
def process_cam(filename):
    ...
    svdir = './vis'
    args = get_args()
    imgname = args.image_path + "/" + filename
    svname = svdir + "/" + filename
    ...
    show_cam_on_image(img, mask,name=svname)
    ...
    cv2.imwrite(svname+'gb.jpg', gb)
    cv2.imwrite(svname+'cam_gb.jpg', cam_gb)
