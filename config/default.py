...
class DefaultConfig:
    ...
    def __init__(self):
        self.PROJECT_NAME = 'star_baseline'  # project name
        self.LOG_DIR = "./log"  # log directory
        self.OUTPUT_DIR = "./output_lightweight"  # saved model directory
        self.DEVICE_ID = "0"  # GPU IDs, i.e. "0,1,2" for multiple GPUs

        self.LOG_PERIOD = 50  # iteration of displaying training log
        self.SLEEP_PERIOD = 5*self.LOG_PERIOD  # iteration of displaying training log
        self.CHECKPOINT_PERIOD = 10  # saving model period
        self.EVAL_PERIOD = 1  # validation period
        self.MAX_EPOCHS = 300  # max training epochs
        self.ISTRAIN = False

        # data
        self.DATA_DIR = "/data_zfs/fast/zhaodong/pulsar_class_resnet50/datadir/"  # dataset path
        self.DATALOADER_NUM_WORKERS = 4  # number of dataloader workers
        self.SAMPLER = 'triplet'  # batch sampler, option: 'triplet','softmax'
        self.BATCH_SIZE = 16#16  # MxN, M: number of persons, N: number of images of per person
        self.NUM_IMG_PER_ID = 8#4  # N, number of images of per person

        # model
        self.INPUT_SIZE = [256, 768]  # HxW set as 600*600 NTS
        ...
        self.LOSS_LABELSMOOTH = 'off' #'on'  # using labelsmooth, option: 'on', 'off'
        self.COS_LAYER = False

        # solver
        self.group_wise = False
        self.OPTIMIZER = 'Adam'  # optimizer

        #self.OPTIMIZER = 'SGD'
        self.BASE_LR = 0.005  # 0.0008  # base learning rate
        ...
        self.STEPS = [30, 60, 90, 120, 150, 200, 250] # for NAbirds
        #self.STEPS = [60, 120, 160]  # best performance
        # self.STEPS = [80, 140, 190]
        self.GAMMA = 0.2  # decay factor of learning rate
        self.WARMUP_FACTOR = 0.01
        self.WARMUP_EPOCHS = 0  # warm up epochs
        ...
        self.FEAT_NORM = "yes"
        self.TEST_WEIGHT = './output/resnet50_30.pth'
