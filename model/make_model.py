...
from .backbones.resnet import resnet50_bap,resnet101_bap
...
class Backbone(nn.Module):
    ...
    def forward_multi(self, x, label=None):  # label is unused if self.cos_layer ==
        ...
        #_, vis_ret = self.ATT.NMS_crop(attention, feature)



        # print (crop_part.size())
        # print (vis_ret.size())
        ...
        # print (feat.size())
        '''
        global_feat = nn.functional.avg_pool2d(feature, feature.shape[2:4])
        global_feat = global_feat.view(global_feat.shape[0], -1)  # flatten to (bs, 2048)
        # feat = feat.mean(0).view(b,-1)
        feat = global_feat#self.bottleneck(global_feat)
        ...
        cls_score = self.classifier(feat)
        return cls_score#, cls_1, cls_2, cls_3, cls_3  # global feature for triplet loss
