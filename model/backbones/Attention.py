...
class BAP(nn.Module):
    ...
    def forward(self, feature, attention):
        ...
        pool_feature = raw_feature * 100

        return raw_feature, pool_feature.view(feature_shape[0], feature_shape[1], -1)  # b*c* 32 part


class DBAP(nn.Module):
    def __init__(self):
        super(DBAP, self).__init__()
    ...
    def forward(self, feature, attention):
        ...
        phi = phi.view(feature_shape[0], -1)
        raw_feature = torch.nn.functional.normalize(phi, dim=-1)
        channel_feature = raw_feature * 100
...
class GDA(nn.Module):
    def __init__(self, n_part=512):
        ...
        self.alpha = nn.Linear(28 * 28, 1, bias=False)  # Parameter(torch.zeros(1),requires_grad=True)
        self.gap_resize = nn.AdaptiveAvgPool2d(28)
        self.gap_all = nn.AdaptiveAvgPool2d(1)
        self.n_part = n_part
...
class ATT():
    ...
    def NMS_crop(self, attention_maps, input_image):
